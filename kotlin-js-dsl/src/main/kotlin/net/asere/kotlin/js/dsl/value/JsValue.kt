...
interface JsValue : JsElement

fun JsValue.raw(syntax: JsSyntax) = JsSyntax("$this.$syntax")
fun JsValue.raw(syntax: String) = JsSyntax("$this.$syntax")
operator fun JsValue.get(value: String) = JsSyntax("$this[${JsString.value(value)}]")
operator fun JsValue.get(value: Number) = JsSyntax("$this[${JsNumber.value(value)}]")
